// src/Vanalytics.Web/src/components/session/SessionDetailModal.tsx
...
export default function SessionDetailModal({ sessionId, onClose, onDeleted }: SessionDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={onClose}>
      <div
        className="relative w-full max-w-6xl max-h-[90vh] overflow-y-auto bg-gray-900 rounded-lg border border-gray-700 p-6 mx-4"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-gray-200 text-2xl leading-none transition-colors"
          aria-label="Close"
        >
          &times;
        </button>
