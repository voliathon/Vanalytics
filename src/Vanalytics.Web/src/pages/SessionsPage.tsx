// src/Vanalytics.Web/src/pages/SessionsPage.tsx
...
export default function SessionsPage() {
  ...
  useEffect(() => {
    api<CharacterOption[]>('/api/characters')
      .then(setCharacters)
      .catch(() => setCharacters([]))
  }, [])
  ...
  useEffect(() => {
    setLoading(true)
    let url = `/api/sessions?page=${page}&pageSize=${pageSize}`
    if (selectedCharacterId) {
      url += `&characterId=${selectedCharacterId}`
    }
    api<SessionListResponse>(url)
      .then((data) => {
              ...
              <tbody className="divide-y divide-gray-800">
                {sessions.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      {s.characterName} <span className="text-gray-500">on {s.server}</span>
                    </td>
